...
function show_slide(slide) {
    ...
    switch (slide) {
        case 1: 
            var mt = build_simple_graph(0, 0, 0, 0);
        break;
        ...
        case 3:
            ...
            view_config = {range: [[-5, 5], [-5, 5]], scale: [1.5, 1.5], position: [0, 0]};

            var view = build_now(mathbox_config, view_config);

            var parser = math.parser();

            var equation = 'y = f(x) = (-2x+10)/4';
            ...
            var yScale = view.scale( {axis: 2, divide: 10, nice:true, zero:false} );
            ...
            var grid = view.grid( {width: 5, divideX: 10, divideY: 10, opacity:0.7, color: 0xb0b0b0});

            view.array({ data: [[5, 0.7, 0], [0.5, 5, 0],],
                channels: 3, 
                live: false,
            ...
            view.array({ data: [[1,2],[-3,4]], channels: 2, live: false, })
                .text({ data: ["A","B"], weight: 'bold', detail: 20 })
                .point({ size: 26, color: "red", })
                .label({ color: 'red', zIndex: 1, });  

            var data = view.interval({
                expr: function (emit, x, i, t) {
                  var func = f1(x);
                  if(func < 10 && func > -11.5)
                    emit(x, func);
                else
                    emit(null);
                },
                width: 128, channels: 2,
              })
                         
            view.line({width: 7, color: '#3090FF', });

            view.array({data: [[4, 1.5, 0]], channels: 3, live: false })
                .text({ data: ["p"], weight: "bold", })
        ...
        case 5:
            ...
            view_config = {range: [[-6, 6], [-6, 6]], scale: [1.5, 1.5], position: [0, 0]};
            ...
            view.array({ data: [[1,2],[-3,4],[5,0],[-1,3]], channels: 2, live: false, })
                .text({ data: ["A","B","D","E"], weight: 'bold', detail: 20 })
                .point({ size: 26, color: "red", })
                .label({ color: 'red', zIndex: 1, });  
        ...
        case 7:
            ...
            view.array({ data: [[1,2],[-3,4],[1,1]], channels: 2, live: false, })
                .text({ data: ["A","B","C"], weight: 'bold', detail: 20 })
                .point({ size: 26, color: "red", })
                .label({ color: 'red', zIndex: 1, });  
